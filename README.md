...
AWS 没有"跨境优选 IP"服务。EC2 自动分配的公网 IPv4 无法转成 EIP；EIP 分配器会反复返回同一地址，且每 Region 仅 5 个配额，用不来筛选。可靠做法是批量启动临时 EC2，从中国大陆视角拨测其自动分配的公网 IPv4，保留最优实例、终止其余。该实例持续运行时这个 IP 一直可用：reboot 保留 IP，stop/start 会更换 IP。
...
参数依次为 region、每轮候选数、最大轮次、保留数；其后可追加任意 CLI 参数，如 `--protect`、`--enable-backend itdog`。存在 `config.yaml` 时脚本会显式传入；直接调用 `python -m crossborder_selector.cli` 时，未指定 `--config` 且当前目录存在 `config.yaml` 会自动读取并打印 `using config.yaml`。
...
| itdog | 三网家宽 → 候选 IP（仅 ping） | 关 | 否 | 非官方接口，随时可能失效；只做 ping 不做 tcping；失败只降级不阻塞 |

reverse 的 `targets` 每项可写 `host` 或 `host:port`：ping 只用 host 部分，tcping 用条目端口，未给端口则回退到 `tcping_port`。默认三网目标里纯 IP 是运营商公共 DNS（走 53），域名走 `tcping_port`（443），避免对 DNS 服务器 tcping 443 得到结构性零分。
...
- 目标 Region 存在默认 VPC；缺失时在 `config.yaml` 手工指定 `subnet_id` 与 `security_group_id`。
...
- winner 的 `InstanceInitiatedShutdownBehavior` 已由默认 `terminate` 改为 `stop`，即使在 OS 内执行 `shutdown`/`poweroff` 也只会停机、不会终止实例，从而保住该 IP。
...
- 加 `--protect` 会对 winner 开启 `DisableApiStop` 与 `DisableApiTermination`；手动解除用 `aws ec2 modify-instance-attribute --instance-id <id> --no-disable-api-termination`（stop 保护同理用 `--no-disable-api-stop`）。
...
单元测试、dry-run 验证、真实冒烟、清理与中断恢复的完整步骤见 [TESTING.md](TESTING.md)。最短一步是 `. .venv/bin/activate && pytest -q`，当前 101 个测试全部通过，不访问网络、不需 AWS 凭证。

## 12. Claude Code skill 与 UI 页面

- Claude Code skill：`.claude/skills/crossborder-select/SKILL.md`。该 skill 让 Claude Code 代运维执行 dry-run、select、cleanup，并读取生成的报告向用户说明结果。
- 报告查看页：`ui/report-viewer.html`。单文件 HTML，本地加载 `out/<run-id>/report.json` 或 `candidates.csv`，展示 winners、每轮结果、全部候选与 prefix 统计，并提供命令构造器。
